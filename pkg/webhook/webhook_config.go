...
var (
	webhookConfigName = "simon-migration-webhook"
	webhookInjectPath = "/mutate"
)

func createOrUpdateMutatingWebhookConfiguration(kubeClient *kubernetes.Clientset, caPEM *bytes.Buffer, webhookService, webhookNamespace string) error {
	log.Info("Initializing the kube client...")

	mutatingWebhookConfigV1Client := kubeClient.AdmissionregistrationV1()

	log.Infof("Creating or updating the mutatingwebhookconfiguration: %s", webhookConfigName)
	...
	mutatingWebhookConfig := &admissionregistrationv1.MutatingWebhookConfiguration{
		...
		Webhooks: []admissionregistrationv1.MutatingWebhook{{
			Name:                    "mutating-webhook.simon.migration",
	...
	if err != nil && apierrors.IsNotFound(err) {
		if _, err := mutatingWebhookConfigV1Client.MutatingWebhookConfigurations().Create(context.TODO(), mutatingWebhookConfig, metav1.CreateOptions{}); err != nil {
			log.Warnf("Failed to create the mutatingwebhookconfiguration: %s", webhookConfigName)
			return err
		}
		log.Infof("Created mutatingwebhookconfiguration: %s", webhookConfigName)
	} else if err != nil {
		log.Warnf("Failed to check the mutatingwebhookconfiguration: %s", webhookConfigName)
		return err
	} else {
		...
		if len(foundWebhookConfig.Webhooks) != len(mutatingWebhookConfig.Webhooks) ||
			...
			if _, err := mutatingWebhookConfigV1Client.MutatingWebhookConfigurations().Update(context.TODO(), mutatingWebhookConfig, metav1.UpdateOptions{}); err != nil {
				log.Warnf("Failed to update the mutatingwebhookconfiguration: %s", webhookConfigName)
				return err
			}
			log.Infof("Updated the mutatingwebhookconfiguration: %s", webhookConfigName)
		}
		log.Infof("The mutatingwebhookconfiguration: %s already exists and has no change", webhookConfigName)
	}
